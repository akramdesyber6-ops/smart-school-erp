...
begin
  foreach table_name in array array[
    ...
    execute format(
      ...
      format('erp_%s_read', table_name), table_name
    );
    execute format(
      ...
      format('erp_%s_write', table_name), table_name
    );
  end loop;
end;
$$;
...
create policy erp_schools_read on public.schools for select using (policies.can_access_school(id));
create policy erp_schools_write on public.schools for all
  using (policies.is_school_admin(id)) with check (policies.is_school_admin(id));
...
create policy erp_class_subjects_read on public.class_subjects for select using (
  exists (
    select 1 from public.classes c
    where c.id = class_subjects.class_id and policies.can_access_school(c.school_id)
  )
);
create policy erp_class_subjects_write on public.class_subjects for all using (
  exists (
    select 1 from public.classes c
...
create policy erp_student_parents_read on public.student_parents for select using (
  exists (
    select 1 from public.students s
    where s.id = student_parents.student_id and policies.can_access_school(s.school_id)
  )
);
create policy erp_student_parents_write on public.student_parents for all using (
  exists (
    select 1 from public.students s
